...
This directory contains six versions of a shared library calculating Fibonacci sequence, all sharing the [same interface (API)](fib.h):
 ...
 * [`fib_5`](fib_5.c) implements Binet's formula (using the golden ration) and therefore has constant time complexity O(1) – since this examples requires math functions like square root, the library itself is linked against `libm.so`, therefore demonstrates *Luci*s ability to handle versions with changed dependencies or its usage of new external functions.
...
There is a second binary [`run-measure`](measure.c) with the ability to measuring the duration of the `fib` library function call.
...
This will create the two executables (prefixed with `run-`) and a subfolder for each library version, containing the corresponding `libfib.so` file – and a symlink to the first version it in the base directory, which is used by the binaries.
...
It is not necessary to sequentially increase the versions, you can directly apply version 6 (here an example running `run-measure` with two consecutive updates):
...
> **Please note:** a rollback to a previously loaded library version is disabled by default (for convenience reasons in *Luci*) – to work around this limitation, you have to set the environment variable `LD_FORCE_UPDATE=1`.
...
Without full RELRO, calling new functions (like the math functions `sqrtl` and `powl` in `fib_5`) wouldn't be possible because it changes the size of the GOT (➔ different writable section makes it incompatible).
