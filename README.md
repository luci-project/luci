...
If the binary is executed using the *Luci* dynamic linker/loader (for example, as parameter or by modifing the interpreter string in the ELF file) and dynamic updates are enabled (`export LD_DYNAMIC_UPDATES=1`), the shared libraries are monitored on the file system.
...
If the update is not possible or *Luci* detects the usage of outdated code, it can inform the user.

Further features, like [interactive/live programming](https://en.wikipedia.org/wiki/Interactive_programming) are currently in an early development stage.


Build
-----

First make sure that you are using an `x86_64` architecture, a Linux Kernel 4.11 or newer, and a compatible distribution (e.g., Debian and Ubuntu, see below), have common build utilites (`gcc`/`g++`, `make` etc) installed and check if all submodules are (recursivly) checked out - for Debian, you can run

    apt-get install build-essential cpplint file gcc g++ libcap2-bin libc++-dev make python3 python3-pyparsing
    git submodule update --init --recursive

If you just want to use it on your system, we recommend to create the directory `/opt/luci` (and give permission to your user)
...
or change its interpreter to the `ld-luci.so` path, during compilation with `-Wl,--dynamic-linker=/opt/luci/ld-luci.so` or with the `elfo-setinterp` tool
    ...
    ./bean/elfo/elfo-setinterp ~/example/a.out /opt/luci/ld-luci.so
...
You are able to control the behaviour not only by the configuration file but also using environment variables.
...
If you want to test *Luci*s dynamic update capabilities with different languages like Ada, Fortran, Go, Pascal and Rust, make sure to have the correspondend compilers installed
