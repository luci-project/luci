...
bool MemorySegment::map() {
	...
	if (mmap.failed()) {
		LOG_ERROR << "Mapping " << target.page_size() << " Bytes at " << reinterpret_cast<void*>(target.page_start()) << " failed: " << mmap.error_message() << endl;
		return false;
	} else if (mmap.value() != target.page_start()) {
		LOG_ERROR << "Requested mapping at " << reinterpret_cast<void*>(target.page_start()) << " but got " << mmap.value() << endl;
		return false;
	}
	target.effective_protection = protection;

	if (copy && source.size > 0) {
		LOG_DEBUG << "Copy " << source.size << " Bytes from " << reinterpret_cast<void*>(source.offset) << " to "  << reinterpret_cast<void*>(target.address()) << endl;
...
uintptr_t MemorySegment::compose() {
	if (buffer == 0) {
		...
		// Create anonymous writable mapping with same size as source
			...
			LOG_DEBUG << "Created compose back buffer at " << reinterpret_cast<void*>(buffer) << " for " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes)" << endl;
		} else {
			LOG_ERROR << "Mapping back buffer for " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) failed: " << mmap.error_message() << endl;
		}
	}
...
bool MemorySegment::finalize(bool force) {
	if (!force && target.status == MEMSEG_INACTIVE) {
		LOG_WARNING << "Memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) is currently disabled, hence ignoring finalize request" << endl;
		return true;
	} else if (buffer != 0) {
		bool result = true;
		// Adjust permissions
		if (auto mprotect = Syscall::mprotect(buffer, target.page_size(), target.protection)) {
			...
			LOG_WARNING << "Unable to adjust protection for " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) of " << source.object << ": " << mprotect.error_message() << endl;
			result = false;
		}
		...
		if (auto mremap = Syscall::mremap(buffer, target.page_size(), target.page_size(), MREMAP_MAYMOVE | MREMAP_FIXED, target.page_start())) {
			...
			buffer = 0;
			LOG_INFO << "Updated " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) of " << source.object << " with composite back buffer (private mapping)" << endl;
		} else {
			LOG_ERROR << "Protecting " << target.page_size() << " Bytes at " << reinterpret_cast<void*>(target.page_start()) << " failed: " << mremap.error_message() << endl;
			result = false;
		}
		return result;
	} else if (target.status == MEMSEG_NOT_MAPPED) {
		LOG_WARNING << "Cannot protect " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) of " << source.object << " since it is not mapped!" << endl;
		return false;
		...
		LOG_ERROR << "Protecting " << target.page_size() << " Bytes at " << reinterpret_cast<void*>(target.page_start()) << " failed: " << mprotect.error_message() << endl;
		return false;
	}
}
...
bool MemorySegment::disable() {
	if (target.status == MEMSEG_NOT_MAPPED) {
		LOG_WARNING << "Cannot disable " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) since it is not mapped!" << endl;
		return false;
	} else if (target.status == MEMSEG_INACTIVE) {
		LOG_DEBUG << "Memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) already inactive!" << endl;
		return true;
	} else {
		if ((target.protection & ~(PROT_NONE | PROT_READ | PROT_EXEC)) != 0) {
			LOG_WARNING << "Memory at " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) has potential unsuitable permissions for disabling!" << endl;
		}
		...
		if (identity.loader.config.detect_outdated == Loader::Config::DETECT_OUTDATED_VIA_USERFAULTFD) {
			...
			if (auto ioctl = Syscall::ioctl(identity.loader.userfaultfd, UFFDIO_REGISTER, &reg)) {
				LOG_DEBUG << "Registered Userfault for memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes)!" << endl;
			} else {
				LOG_ERROR << "Registering " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) for userfault failed!" << endl;
				return false;
			}
			...
			if (auto mremap = Syscall::mremap(target.page_start(), target.page_size(), target.page_size(), MREMAP_MAYMOVE | MREMAP_FIXED | MREMAP_DONTUNMAP, page_start)) {
				...
				target.effective_protection = target.protection;
				LOG_DEBUG << "Memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) disabled, contents moved to " << reinterpret_cast<void*>(mremap.value()) << endl;
				...
				if (auto mprotect = Syscall::mprotect(buffer, target.page_size(), PROT_READ | PROT_WRITE); mprotect.failed()) {
					LOG_ERROR << "Making compose back buffer " << reinterpret_cast<void*>(buffer) << " (" << target.page_size() << " Bytes) writeable failed: " << mprotect.error_message() << endl;
				}
			} else {
				LOG_ERROR << "Remapping " << target.page_size() << " Bytes at " << reinterpret_cast<void*>(target.page_start()) << " failed: " << mremap.error_message() << endl;
				return false;
			}
		}
		return true;
...
bool MemorySegment::enable() {
	if (target.status != MEMSEG_INACTIVE) {
		LOG_WARNING << "Memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) not inactive" << endl;
		return false;
	} else if (buffer == 0) {
		LOG_ERROR << "No back buffer for memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes)" << endl;
		return false;
	} else if (!finalize(true)) {
		LOG_WARNING << "Cannot enable memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes)" << endl;
		return false;
	} else {
		LOG_DEBUG << "Manually enabled memory segment " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes)" << endl;
		return true;
	}
}
...
bool MemorySegment::unmap() {
	if (target.status == MEMSEG_NOT_MAPPED) {
		LOG_WARNING << "Cannot unmap " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) since it is not mapped!" << endl;
		return false;
	} else {
		auto & identity = source.object.file;
		...
		if (auto munmap = Syscall::munmap(target.page_start(), target.page_size())) {
			...
			LOG_WARNING << "Unmapping " << reinterpret_cast<void*>(target.page_start()) << " (" << target.page_size() << " Bytes) failed: " << munmap.error_message() << endl;
			return false;
		}
	}
}
			...
			if (auto ftruncate = Syscall::ftruncate(tmpfd, static_cast<off_t>(target.page_size()))) {
				...
				if (auto mmap = Syscall::mmap(NULL, target.page_size(), PROT_WRITE, MAP_SHARED, tmpfd, 0)) {
					...
					if (auto munmap = Syscall::munmap(mmap.value(), target.page_size()); munmap.failed())
						LOG_WARNING << "Unmapping " << reinterpret_cast<void*>(mmap.value()) << " (" << target.page_size() << " Bytes) failed: " << munmap.error_message() << endl;
					return tmpfd;
				} else {
...
void MemorySegment::dump(Log::Level level) const {
	...
	logger.entry(level, __BASE_FILE__, __LINE__, __MODULE_NAME__)
	   << "Dump of " << source.object << " (@ " << reinterpret_cast<void*>(source.offset) << ", " << source.size << "B)" << endl
	   << (target.status == MEMSEG_MAPPED ? "currently" : "to be") << " mapped at " << reinterpret_cast<void*>(target.address()) << " (" << target.size << "B "
