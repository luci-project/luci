...
void *realloc(void *ptr, size_t size) {
	// If ptr is nullptr it acts as malloc
	if (ptr == nullptr) {
		return malloc(size);
	}

	// If size is 0 it acts as free
	if (size == 0) {
		free(ptr);
		return nullptr;
	}

	// Otherwise allocate new memory according to size
	void *new_ptr = malloc(size);
	if (new_ptr != nullptr) {
		// Copy contents
		memcpy(new_ptr, ptr, Math::min(size, *reinterpret_cast<size_t *>((uintptr_t)(ptr) - HEADER_SIZE)));
		// Free old allocation
		free(ptr);
	}
	return new_ptr;
